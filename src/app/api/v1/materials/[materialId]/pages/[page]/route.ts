...
export async function GET(
  ...
  try {
    ...
    const { materialId, page } = await params;
    ...
    const fileName = manifest.files[pageNumber - 1];

    if (!fileName) {
      throw new AppError("NOT_FOUND");
    }

    const storageKey = `documents/previews/pages/${material.id}/${fileName}`;

    return new Response(null, {
      status: 200,

      headers: {
        "X-Accel-Redirect": nginxInternalPath(storageKey),
